...
void binaryTreeDA<Item>::addLeft(const Item& entry){
    if(!hasLeft()){
        size_t newleft=(2*curr)+1;// Get the index of where the left node is supposed to be.
        tree[newleft]=entry;
        count++;
    }
}
...
void binaryTreeDA<Item>::addRight(const Item& entry){
     if(!hasRight()){
        size_t newRight=(2*curr)+2;// get the index of where the right node is supposed to be.
        tree[newRight]=entry;
        count++;
    }
}
